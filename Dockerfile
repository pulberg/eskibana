...
RUN wget -q -O - https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-${eskibana_version}-linux-x86_64.tar.gz \
|  tar -zx \
&& mv elasticsearch-${eskibana_version} elasticsearch \
&& mkdir -p ${ES_TMPDIR} ${ES_DATADIR} \
&& wget -q -O - https://artifacts.elastic.co/downloads/kibana/kibana-${eskibana_version}-linux-x86_64.tar.gz \
|  tar -zx \
&& mv kibana-${eskibana_version}-linux-x86_64 kibana \
